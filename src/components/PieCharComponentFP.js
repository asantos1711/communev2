...
import { DASHBOARD_FORMA_PAGO } from '../service/Routes';
...
export default function PieCharComponentFP({access_token}){
    ...
    useEffect(()=>{
        let mounted = true;
        setLoading(true)
        Get({url: `${DASHBOARD_FORMA_PAGO}/${moment(date).format("M")}/${moment(date).format('YYYY')}`, access_token: access_token})
        .then(response=>{
        ...
        <Card className="shadow-sm">
            <Card.Body>
                <div className="d-flex">
                    <Card.Title className="flex-grow-1">Forma de pago</Card.Title>
