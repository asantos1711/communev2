...
import { CLAVE_PRODUCCION_SERVICIO_CFDI_GET, CLAVE_PRODUCCION_SERVICIO_DELETE } from '../service/Routes'
...
export default function ClaveProductoServicioList({auth,handleIsEditing,path}){
    ...
    const DataList = () =>{
        setIsLoading(true)
        Get({url: CLAVE_PRODUCCION_SERVICIO_CFDI_GET, access_token: auth.data.access_token})
        .then(response=>{    
    ...
    const deleteItem = (data) => {      
        setIsDeleteId(true)
        Delete({url: `${CLAVE_PRODUCCION_SERVICIO_DELETE}/${data.id}`, access_token: auth.data.access_token})
        .then(response=>{            
            setIsDeleteId(false) 
    ...
    const columns = [
        ...
        {
            dataField: 'claveProduccionServicio',
