...
import { CUOTA_INICIAL_GET, CARGO_CUOTA_INICIAL_GET_BY_CUOTA } from '../service/Routes'
...
import CuotaInicialGeneradaNueva from './CuotaInicialGeneradaNueva'
import CuotaInicialPorLote from './CuotaInicialPorLote'

export default function CuotaInicialGenerada(){
    ...
    useEffect(()=>{
        Get({url: CUOTA_INICIAL_GET, access_token: auth.data.access_token})
        .then(response=>{
        ...
        .catch(error=>{
            //console.log(error)
            toast.error("No se ha podido cargar los tipos de cuota extraordinaria. Intente más tarde", {autoClose: 8000})
        })
    }, [])
    ...
    const handleChangeRadio = e  =>{
        ...
        Get({url: `${CARGO_CUOTA_INICIAL_GET_BY_CUOTA}/${selectCuotaExtr}/${e.target.value}`, access_token: auth.data.access_token})
        .then(response=>{
            //console.log(response)
    ...
    const onChangeSelect = e =>{
        ...
        if(e.target.value === ""){
            ...
            setIsLoading(true)
            Get({url: `${CARGO_CUOTA_INICIAL_GET_BY_CUOTA}/${e.target.value}/${tipo}`, access_token: auth.data.access_token})
            .then(response=>{
                //console.log(response)
                setCountCondos(response.data.data.countCondos)
                setCountHabitacional(response.data.data.countHabitacion)
                ...
                <div>
                    <Switch>
                            ...
                            <Row>
                                <Col className="mb-4">
                                    <CuotaExtraordinariaSearch
                                        title="Cuotas inicial generadas"
                                        url={url} 
                                        selectOpt={selectOpt} 
                        ...
                        <Route exact path={`${path}/nuevo`}>
                            <CuotaInicialGeneradaNueva access_token={auth.data.access_token} selectOpt={selectOpt}/>
                        </Route>
                        <Route exact path={`${path}/nuevoporlote`}>
                            <CuotaInicialPorLote access_token={auth.data.access_token} selectOpt={selectOpt}/>
                        </Route>
