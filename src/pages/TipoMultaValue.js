...
import { TIPO_MULTA_SAVE,TIPO_MULTA_GET_BY_ID } from '../service/Routes'
...
export default function TipoMultaValue(props){
    ...
    useEffect(()=>{        
        if(idEntity){
            setLoadEntity(true)
            Get({url: `${TIPO_MULTA_GET_BY_ID}/${idEntity}`, access_token: props.auth.data.access_token})
            .then(response=>{
                let entity ={
        ...
        <div>
            {
                ...
                <Formik
                    ...
                    onSubmit={(values, { setSubmitting, setFieldValue }) => { 
                        ...
                        Post({url: TIPO_MULTA_SAVE, data: d,access_token: props.auth.data.access_token, header:true})
                        .then(response=>{
                            setSubmitting(false)
                    ...
                    <Form className="mt-4 form" onSubmit={handleSubmit}>
                        ...
                        <Row>
                            <Col>
                                ...
                                <Link to="/catalogos/tipo-multa" className="btn btn-secondary">Cancelar</Link>
                            </Col>
                        </Row>
                    </Form>
                )}            
