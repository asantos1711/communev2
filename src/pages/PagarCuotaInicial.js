...
import { METODO_PAGO_GET, CARGO_CUOTA_INICIAL_GET_BY_ID, PAGO_CARGO_CUOTA_INICIAL_SAVE, CAJA_GET_ACTIVE } from '../service/Routes'
...
export default function PagarCuotaInicial(){
    const {id} = useParams()
    ...
    useEffect(()=>{
        const urls = [METODO_PAGO_GET, `${CARGO_CUOTA_INICIAL_GET_BY_ID}/${id}`]
        ...
        .then(response=>{
            ...
            setInputMonto(calcAmount(response[1].data.data.monto, response[1].data.data.pagoCuotaInicialList))
            setLoading(false)            
        })
        .catch(error=>{
           // console.log(error)
    ...
    const onClickPayment = e =>{        
        if(validNumberPositive(inputMonto, false)){
            ...
            console.log(d)
            Post({url: PAGO_CARGO_CUOTA_INICIAL_SAVE, data: d, access_token: auth.data.access_token, header: true})
            .then(response=>{
                //console.log(response)
                setSubmiting(false)
                                ...
                                <Card className="shadow">
                                    <Card.Body>
                                        ...
                                        <Row>
                                            <Col xs="6" md="6">
                                                <dl className="row">
                                                    <dt className="col-sm-12">{sancion.cuotaInicial.name}</dt>
                                                    ...
                                                    <dt className="col-sm-9">{sancion.cuotaInicial.descripcion}</dt>
                                                    ...
                                                    <Card>
                                                        ...
                                                        <Accordion.Collapse eventKey="0">
                                                            <Card.Body>
                                                                <ul>
                                                                    {
                                                                        sancion.pagoCuotaInicialList.length === 0
                                                                        ? <li>No hay pagos registrados hasta el momento</li>
                                                                        : sancion.pagoCuotaInicialList.map((item,i)=>(
                                                                            <li key={i} className={item.pagoStatus==="cancelado" ? 'text-danger' : ''}>{moment(item.create_at).format("DD-MMM-YYYY HH:mm")}{' - '}
                                            ...
                                            <Col xs lg={{span: 3, offset: 3}}>
                                                ...
                                                <Form.Group>
                                                    ...
                                                    <InputMask
                                                        className={`${!errorNoCuenta && 'error'} form-control form-control-sm`} 
                                                        mask="9999" 
                                                        onChange={e=>{
                                                            if(e.target.value === ""){
