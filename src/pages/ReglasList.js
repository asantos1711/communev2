...
import {REGLAS_GET, REGLAS_DELETE} from '../service/Routes'
...
export  default function ReglasList({auth,handleIsEditing,path}){
    ...
    const DataList = () =>{
        setIsLoading(true)
        Get({url: REGLAS_GET, access_token: auth.data.access_token})
        .then(response=>{
            setIsLoading(false)
    ...
    const deleteItem = (data) => {      
        setIsDeleteId(true)
        Delete({url: `${REGLAS_DELETE}/${data.id}`, access_token: auth.data.access_token})
        .then(response=>{            
            setIsDeleteId(false)   
    ...
    const percentFormatter = cell =>{
        return(
            <NumberFormat value={cell} displayType={'text'} suffix={'%'} />
        )
    }
