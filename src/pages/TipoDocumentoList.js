...
import { TIPO_DOCUMENTO_GET, TIPO_DOCUMENTO_DELETE } from '../service/Routes'
...
export default function TipoDocumentoList({auth,handleIsEditing,path}){
    ...
    const DataList = () =>{
        setIsLoading(true)
        Get({url: TIPO_DOCUMENTO_GET, access_token: auth.data.access_token})
        .then(response=>{    
    ...
    const deleteItem = (data) => {      
        setIsDeleteId(true)
        Delete({url: `${TIPO_DOCUMENTO_DELETE}/${data.id}`, access_token: auth.data.access_token})
        .then(response=>{            
            setIsDeleteId(false) 
    ...
    const columns = [
        ...
        {
            dataField: 'tipoComprobante.name',
