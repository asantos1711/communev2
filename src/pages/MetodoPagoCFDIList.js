...
import { METODO_PAGO_CFI_GET, METODO_PAGO_CFI_DELETE } from '../service/Routes'
...
export default function MetodoPagoCFDIList({auth,handleIsEditing,path}){
    ...
    const DataList = () =>{
        setIsLoading(true)
        Get({url: METODO_PAGO_CFI_GET, access_token: auth.data.access_token})
        .then(response=>{    
    ...
    const deleteItem = (data) => {      
        setIsDeleteId(true)
        Delete({url: `${METODO_PAGO_CFI_DELETE}/${data.id}`, access_token: auth.data.access_token})
        .then(response=>{            
            setIsDeleteId(false)            
