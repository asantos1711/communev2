...
import { CUOTA_EXTRAORDINARIA_GET_BY_ID, CUOTA_EXTRAORDINARIA_SAVE } from '../service/Routes';
...
export default function CuotaExtraordinariaValue({auth, isEditing}){
    ...
    useEffect(()=>{        
        if(idEntity){
            setLoadEntity(true)
            Get({url: `${CUOTA_EXTRAORDINARIA_GET_BY_ID}/${idEntity}`, access_token: auth.data.access_token})
            .then(response=>{                        
                let entity ={
        ...
        <div>
            {
                ...
                <Formik
                    ...
                    onSubmit={(values, { setSubmitting, setFieldValue }) => {   
                        Post({url: CUOTA_EXTRAORDINARIA_SAVE, data: values,access_token: auth.data.access_token, header:true})
                        .then(response=>{
                            //console.log(response)
                            setSubmitting(false)
                            if(response.data.success){
                                toast.success("Acción exitosa",{ autoClose: 2000 })
                                history.push("/admin/cuota-extraordinaria")
                            }else{
                    ...
                    <Form className="mt-4 form" onSubmit={handleSubmit}>
                        ...
                        <Row>
                            <Col>
                                ...
                                <Link to="/admin/cuota-extraordinaria" className="btn btn-secondary">Cancelar</Link>
                            </Col>
                        </Row>
                    </Form>
                )}            
