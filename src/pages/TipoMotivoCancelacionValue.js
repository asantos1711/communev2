...
import { TIPO_MOTIVO_CANCELACION_GET_BY_ID, TIPO_MOTIVO_CANCELACION_SAVE } from "../service/Routes"
...
export default function TipoMotivoCancelacionValue(props){
    ...
    useEffect(()=>{        
        if(idEntity){
            setLoadEntity(true)
            Get({url: `${TIPO_MOTIVO_CANCELACION_GET_BY_ID}/${idEntity}`, access_token: props.auth.data.access_token})
            .then(response=>{
                let entity ={
        ...
        <div>
            {
                ...
                <Formik
                    ...
                    onSubmit={(values, { setSubmitting, setFieldValue }) => { 
                        ...
                        Post({url: TIPO_MOTIVO_CANCELACION_SAVE, data: d,access_token: props.auth.data.access_token, header:true})
                        .then(response=>{
                            setSubmitting(false)
                            if(response.data.success){
                                setFieldValue("id", response.data.data.id)
                                toast.success("Acción exitosa",{ autoClose: 2000 })
                                history.push("/catalogos/motivo-cancelacion")
                            }else{
