...
export default function NewsLetterValue({ auth }) {
  ...
  const UserShema = Yup.object().shape({
    name: Yup.string().min(3, "Muy corto!").required("Campo Requerido"),
    descripcion: Yup.string().min(20, "Muy corto!").required("Campo Requerido"),
    fileName: Yup.string().required("Campo Requerido"),
  });
          ...
          onSubmit={async (values, { setSubmitting, setFieldValue }) => {
            try {
              ...
              if (initialValues.oldName) {
                const constanciaRef = storageRef.child(
                  `documents/${process.env.REACT_APP_RESIDENCIAL}/${initialValues.oldName}`
                );
              ...
              const fileRef = storageRef.child(
                `documents/${process.env.REACT_APP_RESIDENCIAL}/` + fileName
              );
            ...
            <Form className="mt-4 form" onSubmit={handleSubmit}>
              ...
              <Row>
                ...
                <Col xs="12" lg="6">
                  <Form.Group>
                    ...
                    <div className="input-group">
                      <div className="custom-file">
                        <input
                          ...
                          id="inputFile"
                          aria-describedby="inputFileAddon04"
                          onChange={(e) => {
                            handleUploadFile(e);
                    ...
                    {errorFile && (
                      <Form.Control.Feedback type="invalid">
                        Archivo inválido. Solo archivo en formato JPG, JPEG, PNG
                        permitido
                      </Form.Control.Feedback>
                    )}
                  </Form.Group>
              ...
              <Row>
                <Col>
                  ...
                  <Link to="/admin/newsletter" className="btn btn-secondary">
                    Cancelar
                  </Link>
                </Col>
              </Row>
