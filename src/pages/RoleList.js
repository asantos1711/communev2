...
import { ROLE_GET, ROLE_DELETE } from '../service/Routes'
...
export default function RoleList({auth,handleIsEditing,path}){
    ...
    const DataList = () =>{
        setIsLoading(true)
        Get({url: ROLE_GET, access_token: auth.data.access_token})
        .then(response=>{
            setIsLoading(false)
    ...
    const deleteItem = (data) => {      
        setIsDeleteId(true)
        Delete({url: `${ROLE_DELETE}/${data.id}`, access_token: auth.data.access_token})
        .then(response=>{            
            setIsDeleteId(false)            
